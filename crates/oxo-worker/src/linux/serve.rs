...
fn run(config: Config) -> Result<(), String> {
    // native worker deliberately initializes Ruby and loads the Rack app before creating
...
fn start_listener_and_threads(config: Config, queue: Arc<JobQueue>) -> Result<(), String> {
    ...
    // Report parsed worker settings on stderr. The supervisor reserves stdout
    // for readiness, so configuration logging must not interfere with that handshake.
    eprintln!(
    ...
    for stream in listener.incoming() {
        match stream {
            Ok(stream) => {
                ...
                thread::spawn(move || {
                    // a panic in the per-connection handler must be observable, not a
