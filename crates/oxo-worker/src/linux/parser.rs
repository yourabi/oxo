...
pub(super) fn parse_request(
    ...
    // Use one buffered reader for headers and body so bytes read ahead with the
    // headers remain available to the body parser. Validate lines byte by byte with
    // strict CRLF and size bounds; direct HTTP connections serve one request only.
    ...
    if len > 0 {
        ...
        // prefetched are consumed here, never discarded.
        reader
            .read_exact(&mut body)
...
// Read from the caller's buffered reader while preserving strict bare-LF
// rejection and incremental header-size checks. Prefetched body bytes stay
// in that same reader.
...
mod tests {
    ...
    // the BufReader now serving head+body must never lose body bytes
