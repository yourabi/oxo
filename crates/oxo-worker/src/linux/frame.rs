//! Persistent binary-frame worker connections sharing the native worker JobQueue.
//!
//! Decode complete bounded frames before dispatch. The listener distinguishes
//! binary frames from direct HTTP requests without consuming the first byte.
//! Connection ownership and shutdown tracking stay with the worker; complete
//! responses may reuse the socket, while malformed or interrupted exchanges close.
//! Blocking Ruby execution retains the native worker's interruption limitations.
...
pub(super) fn try_handle_frame_connection(
    ...
    // HTTP path is byte-identical. Blocking recv: waits for ≥1 byte or EOF.
...
///
/// **Defense-in-depth ( env-parity):** even though the edge already stripped
