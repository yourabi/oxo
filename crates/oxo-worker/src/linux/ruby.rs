...
// Cache the Ruby helper module after initialization. The registered Lazy handle
// keeps it visible to Ruby GC and avoids repeated constant lookup per request.
...
module Oxo
  ...
  def build_env(method, path, query, server_name, server_port, scheme, remote_addr, multithread, multiprocess, header_flat, body)
    ...
    # Keep this env Rack::Lint-clean: native worker does not expose rack.hijack keys. The
