...
// The legacy hyper-edge hardening defaults. Each maps to a `hyper`/`tokio`
// knob used by `oxo-edge`, not the current Pingora edge. A uniform sentinel
// of `0` means "disabled / unlimited / leave hyper's default" for every knob.
// Keep this generation boundary explicit so legacy comments do not become
// Pingora TLS/H2 claims.
...
pub struct RackRequest {
    ...
    /// `SERVER_PORT`.
    pub server_port: u16,
    /// `rack.url_scheme` for the legacy core/hyper request model. The current
...
impl Config {
    /// `true` when bound to a non-loopback interface. The legacy hyper binary logs
    /// a loud warning for protections that belong to the newer Pingora public edge.
...
pub enum ConfigError {
    ...
    #[error(
        "refusing to bind non-loopback address {0} without OXO_INSECURE_PUBLIC_BIND=1 — \
         timeouts and connection/slowloris caps now exist, but there is still NO TLS \
         (plaintext), so Oxo MUST run behind a trusted TLS-terminating boundary until \
         the TLS milestone"
    )]
...
/// The single source of truth for "a client-supplied header that could forge the client's
/// forwarding/real-IP identity." Any header matching this is DROPPED before it can become a
/// Rack `HTTP_*` env key — because identity (`REMOTE_ADDR`, scheme, host) is set ONLY from a
/// trusted side-channel (the edge's `x-oxo-*` fields on the HTTP hop, or the native frame
/// fields on the binary hop), never from a client header.
///
/// Why centralized: this exact denylist was previously maintained as three drifting copies
/// (the HTTP parser, the frame decoder, the edge strip) plus a partial fourth in the edge
/// identity resolver. They disagreed and omitted `Client-IP` + the CDN client-IP family, so a
/// client sending only `Client-IP: 1.2.3.4` reached `HTTP_CLIENT_IP`, which Rails
/// `ActionDispatch::RemoteIp` trusts by default (its spoof guard only fires when
/// `X-Forwarded-For` is also present — which we strip) → `request.remote_ip` was spoofable.
///
/// This is a comprehensive, centralized DENYLIST, not an allowlist: arbitrary application
/// headers (User-Agent, Accept, custom `X-App-*`) must pass through untouched, so an allowlist
/// is infeasible. It is complete against the known consumer — Rails `RemoteIp` default-trusts
/// `HTTP_CLIENT_IP` + `HTTP_X_FORWARDED_FOR`, both covered — and additionally covers the common
/// CDN/provider client-IP headers below. **New provider headers are a periodic-review
/// obligation** (see `docs/THREAT_MODEL.md`).
///
/// `name` MUST already be ASCII-lowercased (every call site lowercases first). This
/// deliberately does NOT match the `x-oxo-*` reserved namespace — that is a separate class
/// dropped by its own arm at each call site.
