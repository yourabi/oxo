//! W0 — the async-kind zero-replay contract on the frame hop.
//!
//! The stale-reuse retry carve-out (frame_hop.rs `StaleBeforeDispatch`) assumes
//! zero-response-byte EOF on a reused pooled connection means the worker NEVER
//! dispatched. For the classic S1 worker that inference is sound; for a fiber-reactor
//! worker it is ambiguous under crash/drain-abandonment (GRADUATION_DESIGN §3c): a
//! killed reactor closes every in-flight connection with zero response bytes, and the
//! carve-out's reconnect-or-failover path silently REPLAYS a dispatched, possibly
//! non-idempotent request into a sibling.
//!
//! Contract pinned here: under `OXO_WORKER_KIND=async` a stale reused connection is
//! a terminal 502 — no fresh-connect retry, no sibling failover. The classic default
//! keeps the shipped carve-out (its behavior is pinned too, so the kind gate is proven
//! differential, not accidental).
...
/// 0xBF Full response frame, prebuilt (the C6 codec: 6-byte prefix + envelope).
...
/// CLASSIC DEFAULT: the shipped carve-out is pinned — stale reuse retries, the dead
/// reconnect fails over, and the sibling serves the request. (This is the documented
/// classic behavior the async gate must NOT change.)
#[test]
