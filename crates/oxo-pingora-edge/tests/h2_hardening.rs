...
//! the H1-only keepalive caps do NOT apply to H2 (the H1-only limit). Requires
...
fn h2_rapid_reset_flood_triggers_goaway() {
    // flooding HEADERS+RST_STREAM past --h2-max-reset-streams must trip the server's
...
fn h2_connection_ignores_h1_keepalive_and_request_caps() {
    // (H1-only limit): the keepalive idle timeout and
