...
fn service_runner_terminates_edge_when_worker_respawn_fails() {
    // when a crashed worker cannot be respawned (spawn error, or the respawned worker
    ...
    // The discriminating assertion: the edge must NOT be orphaned. After the service
...
fn service_runner_honors_shutdown_promptly_during_worker_restart() {
    // a drain signal that arrives while a worker is being restarted must be honored
...
#[test]
fn service_runner_starts_rails_gruf_grpc_fixture_and_serves_unary_call() {
    let fixture = Fixture::new("rails-gruf-grpc");
    let rails_root = ruby_fixture::workspace_root().join("test/fixtures/rails_app");
    let app = rails_root.join("config.ru");
    let gemfile = rails_root.join("Gemfile");
    let grpc_bin = rails_root.join("bin").join("oxo-grpc");
    assert!(grpc_bin.exists(), "missing {}", grpc_bin.display());
    ...
    cmd.env("BUNDLE_GEMFILE", &gemfile)
        ...
        .env("OXO_EDGE_SERVER_NAME", "app.example")
        .env("OXO_GRPC_ENABLED", "1")
        .env("OXO_GRPC_BIN", &grpc_bin)
        .env("OXO_GRPC_BIND", format!("127.0.0.1:{grpc_port}"))
        .env("OXO_GRPC_APP", "oxo_fixture")
        .env("OXO_GRPC_CONFIG", "grpc/oxo_fixture.rb");
    for (name, value) in ruby_fixture::command("ruby").get_envs() {
        if let Some(value) = value {
            cmd.env(name, value);
        }
    }
    let mut child = process_fixture::CapturedChild::new(cmd.spawn().expect("spawn service runner"));
    let deadline = Instant::now() + Duration::from_secs(30);
    while TcpStream::connect(("127.0.0.1", edge_port)).is_err()
        || TcpStream::connect(("127.0.0.1", grpc_port)).is_err()
    {
        assert!(
            child.poll().is_none() && Instant::now() < deadline,
            "Rails/gruf service did not become ready: {}",
            ruby_fixture::diagnostic(&child.stderr())
        );
        thread::sleep(Duration::from_millis(20));
    }

    let output = run_grpc_fixture_client(&fixture.root, &rails_root, grpc_port);
    assert!(
    ...
    assert!(stdout.contains("metadata=service-runner"), "{stdout}");
    assert!(
        stdout.contains("service=OxoGrpc::FixtureService"),
        "{stdout}"
    );
    ...
    assert!(!timed_out, "runner hung after rails grpc SIGTERM");
    assert!(output.status.success(), "{:?}", output.status);
    assert_worker_socket_is_not_live(&fixture.socket);
}

fn run_grpc_fixture_client(temp_dir: &Path, rails_root: &Path, port: u16) -> Output {
    let client = temp_dir.join("grpc-client.rb");
    fs::write(
        &client,
        r#"
require_relative File.join(ARGV.fetch(0), "grpc", "oxo_fixture")
stub = OxoGrpc::FixtureService.rpc_stub_class.new("127.0.0.1:#{ARGV.fetch(1)}", :this_channel_is_insecure)
reply = stub.ping(
  OxoGrpc::PingRequest.new("message" => "runner"),
  metadata: { "x-oxo-test" => "service-runner" }
)
