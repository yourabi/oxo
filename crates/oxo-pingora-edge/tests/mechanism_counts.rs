//! Linux request-cost checks using syscall deltas and optional allocation counters.
//!
//! Run separate edges under strace for N and 3N requests and compare their counts.
//! For deterministic operations, count(3N) - count(N) should equal 2N times the
//! per-request count. This relies on comparable startup and shutdown work. strace
//! is the edge's parent, avoiding attach restrictions under ptrace_scope=1.
//!
//! Socket and scheduler counts vary with readiness and task scheduling; their
//! checks use explicit bounds rather than exact per-request equality.
...
/// Trace either worker-hop format with the selected downstream connection policy.
fn spawn_edge_under_strace_hop(
    ...
    cmd.arg("-c")
        ...
        // Use one traced edge thread to bound startup overhead. These assertions
        // cover per-request operations; this setting does not measure scaling.
...
/// With a Rails `/` fallthrough mount, a dynamic request checks deployment
/// identity and resolves the path once. Cached process identity avoids repeated
/// getpid calls. Assert each deterministic count from the two-instance delta.
#[test]
fn syscall_ledger_dynamic_route_with_root_fallthrough_mount() {
    ...
    assert_eq!(
        delta(&hi, &lo, "getpid"),
        0,
        "cached pid requires no per-request getpid call"
    );
    assert_eq!(
    ...
    assert_eq!(
        delta(&hi, &lo, "openat2"),
        per_req,
        "crenel resolution must occur exactly once per request"
    );
}

/// Trace the binary frame hop with downstream keepalive and pooled worker
/// connections. Print all per-request syscall deltas; assert only the named bounds.
#[test]
fn syscall_ledger_frame_hop_keepalive_reuse_path() {
    ...
    // Checkout uses reactor-cached readiness, issuing a read only when signaled.
    // The bounds below detect added socket reads or total syscall growth while
    // allowing scheduling-dependent variation.
...
fn syscall_ledger_dynamic_route_without_mounts_has_zero_fs_probes() {
    ...
    assert_eq!(
        delta(&hi, &lo, "getpid"),
        0,
        "getpid stays at zero per request"
    );
    assert_eq!(
        delta(&hi, &lo, "newfstatat"),
        0,
...
/// A fake worker for the binary frame protocol. Precompute its fixed response
/// with the six-byte magic/version/length prefix. The connection loop accepts
/// multiple sequential requests so the edge can exercise pooling.
fn canned_response_frame() -> Vec<u8> {
    canned_response_frame_with(&[("content-type", "text/plain")], b"ok")
}

/// parameterized so the header-rich ruler can exercise multi-header response
...
// Fixed binary response bytes used by allocation and frame-hop syscall checks.
fn spawn_frame_worker_many(listener: UnixListener) -> mpsc::Receiver<()> {
    spawn_frame_worker_serving(listener, canned_response_frame())
}

/// worker serving an arbitrary canned frame (the header-rich ruler's worker).
...
/// Count allocation events over 200 one-shot requests in an alloc-count build.
/// Compare the amortized value with the explicit budget; it is a count of
/// allocator calls, not a measurement of allocation time or bytes.
...
fn alloc_events_per_request_stay_under_pinned_budget() {
    ...
    // Print the measured count as well as checking its budget for regression diagnosis.
    eprintln!(
        "alloc_events_per_request={per_request:.2} (n={n}, delta={})",
        after - before
    );
    // Allocation-event budget for this fixture and connection policy.
    const ALLOC_EVENTS_PER_REQUEST_BUDGET: f64 = 98.0;
    assert!(
...
/// Measure allocation events on the pooled binary worker hop using one-shot
/// client connections. Keep the fixture workload aligned with the HTTP-hop check.
#[cfg(feature = "alloc-count")]
#[test]
fn alloc_events_per_request_frame_hop_stay_under_pinned_budget() {
    ...
    // Allocation-event budget for one-shot clients using the binary worker hop.
    const FRAME_ALLOC_EVENTS_PER_REQUEST_BUDGET: f64 = 61.0;
    assert!(
...
/// Measure allocation events with one client connection reused for all requests.
/// This amortizes connection setup differently from the one-shot check, so the
/// two counts describe distinct workloads and are not interchangeable.
#[cfg(feature = "alloc-count")]
#[test]
fn alloc_events_per_request_frame_hop_keepalive_denomination() {
    ...
    // Allocation-event budget for downstream keepalive and the binary worker hop.
    const KEEPALIVE_ALLOC_EVENTS_PER_REQUEST_BUDGET: f64 = 50.0;
    assert!(
...
/// Header-rich keepalive workload: eight request headers, including mixed-case
/// names, and four response headers. This exercises per-header storage and name
/// handling beyond the minimal-header fixtures.
#[cfg(feature = "alloc-count")]
#[test]
fn alloc_events_per_request_frame_hop_header_rich() {
    ...
    // Allocation-event budget for the header-rich keepalive workload.
    const HEADER_RICH_ALLOC_EVENTS_PER_REQUEST_BUDGET: f64 = 73.0;
    assert!(
...
/// With edge-bench, CPU injection must announce itself at boot and consume
/// CPU. Compare process user/system ticks with and without injection over a fixed
/// request count. The configured dose is deliberately large relative to the
