...
#[test]
fn hyper_public_bind_env_does_not_unlock_pingora() {
    let _guard = serial_test();
    let fixture = Fixture::new("hyper-public-bind-denied");
...
/// Verify TLS 1.3 session resumption with a client-side session store. The first
/// handshake is full and the next must report HandshakeKind::Resumed after tickets
/// have been read. This tests the rustls configuration constructed by Pingora;
/// it does not establish ticket persistence across server restarts.
#[cfg(feature = "tls-rustls")]
#[test]
...
fn public_smoke_beta_tls_h2_serves_and_private_admin_reports_health() {
    ...
    // frame hop is proven end-to-end against the real worker in worker_e2e).
...
fn public_smoke_beta_h2_without_content_length_forwards_body_to_worker() {
    // HTTP/2 can delimit a body with END_STREAM without Content-Length. Send
    // that form and assert the worker receives every byte with a synthesized length.
    let _guard = serial_test();
    ...
    // curl uploads stdin with unknown length, producing HTTP/2 DATA frames
    // terminated by END_STREAM without a content-length header.
...
fn public_smoke_beta_global_in_flight_cap_overloads_before_identity_resolution() {
    ...
    // enforced here because keepalive reuse is off in this test (one-shot).
    assert!(
    ...
    // posture fields: the serve-rails preset is not used in this test.
...
fn static_hostile_paths_reject_before_worker_acquisition() {
    ...
    // never a worker fallthrough (the -F6 sanitizer-vs-fallthrough contract).
    for target in [
        "/assets/../secret.txt",
        "/%2e%2e/etc/passwd",
        "/assets/a%00.css",
...
fn static_capistrano_deploy_flip_serves_new_release() {
    ...
    // After a deployment symlink changes, poll to a deadline for re-pinning to the new release.
...
fn keepalive_request_limit_beats_idle_timeout() {
    // whichever bound fires first wins. With a short request limit and a long
...
fn reject_pre_admission_is_un_metered_and_closes_connection() {
    ...
    assert_eq!(ok.read_response().status, 200);
    // Wait on responses_total, the counter asserted below. Admission occurs
    // earlier, so waiting on admitted_total could read incomplete response accounting.
...
fn keepalive_reused_connection_desync_corpus() {
    // Send each smuggling vector as the second request on a reused connection.
    // Validation must run for every request. Each rejection must return 4xx/413,
    // set Connection: close, reach EOF, and never reach the worker. Include an
    // over-cap body to check that unread input cannot become another request.
    let _guard = serial_test();
...
/// inherits it), never assumed: the calling process's shell limit is not this test's to pick.
#[test]
...
fn serve_rails_preset_serves_assets_keepalive_and_falls_through() {
    // ONE flag = static pair + keepalive + SSE. The asset is edge-served
    ...
    // /ready posture: active, static dir named, idle timeout enforced.
    let ready = String::from_utf8_lossy(&send_tcp(
        admin_port,
        b"GET /ready HTTP/1.1\r\nHost: admin.local\r\n\r\n",
    ))
...
fn serve_rails_missing_public_soft_disables_static() {
    // (owner decision): a root WITHOUT public/ boots with a helpful warning,
    ...
    // --check-config surface ( emitted before the early return).
    let _guard = serial_test();
...
fn serve_rails_missing_root_refuses_boot() {
    // / a missing (typo'd) root is a fail-closed boot refusal
...
fn serve_rails_static_dir_override_wins() {
    // --static-rails-preset names the static dir explicitly; serve-rails
...
fn serve_rails_missing_assets_serves_fallthrough_only() {
    // / root + public exist, public/assets does not (API-only
    ...
    assert!(
        ...
        "missing public/assets must not refuse boot"
    );
    let stderr = String::from_utf8_lossy(&check.stderr);
...
fn serve_rails_sse_streams_and_explicit_env_off_beats_preset() {
    // The preset enables streaming; explicit OXO_EDGE_SSE=0 and
    // OXO_EDGE_KEEPALIVE=0 override it, yielding SSE rejection and one-shot requests.
    let _guard = serial_test();
...
fn serve_rails_does_not_satisfy_public_mode_gates() {
    // ( fail-closed contract): the preset touches only
...
fn serve_rails_check_config_pin_audit_catches_boot_refusals() {
    // --check-config validates static docroots, not just mount-spec syntax.
    // An explicit preset pointing at a missing directory must fail during dry-run.
    let _guard = serial_test();
...
// With edge-bench compiled, the native route answers only when enabled at boot.
// Otherwise it falls through to the worker. The route is absent without the feature.
...
fn edge_bench_route_answers_natively_when_flag_on() {
    ...
    let expected_body = b"oxo-bench-ok";
    assert!(
...
// Deliberately per-suite: the CN/SAN here is load-bearing in this suite's assertions.
#[cfg(feature = "tls-rustls")]
...
// Deliberately per-suite: startup deadlines/diagnostics are fixture-specific.
...
// Deliberately per-suite: return type/read-timeout differ across suites.
