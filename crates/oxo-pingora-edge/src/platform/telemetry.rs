...
/// Per-request stderr logging policy. `Rejections` is the default: log rejected
/// requests without a response log line for every successful request.
...
pub(super) struct EdgeTelemetry {
    ...
    // Sequence of the last request; zero means none. The admin reader reconstructs
    // `oxo-{pid}-{sequence}` using cached_pid, avoiding a per-request string copy.
    ...
    /// flag's outcome (incl. static soft-disable) is not stderr-only.
...
impl EdgeTelemetry {
    ...
    fn ready_json(&self) -> String {
        ...
        format!(
            ...
            // Sticky-dispatch counters are zero unless sticky mode is active.
            // sticky_pins reports the assignment count for each worker.
...
fn handle_admin_connection(mut stream: std::net::TcpStream, telemetry: &EdgeTelemetry) {
    ...
    let (status, content_type, body) = match path {
        ...
        // Pool-health counters have their own endpoint. `registered:false` means
        // no frame pool exists, as with the HTTP worker hop.
        "/pool-health" => (
            "200 OK",
            "application/json",
        ...
        // Allocation-event count for builds with `alloc-count`. A separate endpoint
        // keeps optional diagnostic fields out of the /ready schema.
        ...
        // Hop timing has a separate endpoint available with `hop-timing`.
        // It reports zeros unless `OXO_HOP_TIMING=1` enables recording.
...
/// Combine stage and dispatch timing JSON at the root object. Remove exactly
/// one final closing brace: removing the entire brace suffix would also remove
/// the nested histogram and stage-map delimiters.
#[cfg(feature = "hop-timing")]
