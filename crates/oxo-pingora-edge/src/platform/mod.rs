...
/// M1 / W2: the process id, read once. glibc stopped caching getpid in 2.25, so
/// `std::process::id()` is a real syscall — this was measured at exactly 1.000/request in
/// before the cache. First use runs post-fork by construction (nothing calls it
/// before the first request or the first admin read), so the cached value is correct even
/// if pingora daemon mode were ever enabled. Shared by the request-id producer (proxy)
/// and the last-request-id reconstructor (telemetry), which must agree byte-for-byte.
...
// reproduces the pre-split pub(super) visibility of socket_activation_env_name
