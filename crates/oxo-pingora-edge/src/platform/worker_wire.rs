...
/// headers are collected INTO the request's bump arena (names, values,
    ...
    for h in headers {
        ...
        if lower == "connection" {
            for token in value.split(',') {
                // compare case-insensitively without allocating a lowercase
...
pub(super) async fn read_complete_body(
    ...
    // For HTTP/2 without Content-Length, buffer DATA until END_STREAM while
    // enforcing the body cap. Derive the worker length from those bytes.
