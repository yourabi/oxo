...
/// Worker-hop wire format: pooled binary frames by default, or one-shot HTTP/1.1.
...
pub(super) struct RunEdgeOptions {
    ...
    /// resolved serve-rails preset posture for /ready.
...
/// Static-routing candidate check. Without a pin set, probe GET/HEAD requests
/// so newly deployed files remain visible. With a pin set, match the first path
/// segment. Percent-encoded segments remain candidates because decoding could
/// produce a pinned name; canonical path validation still belongs to crenel.
...
pub(super) fn run_edge(edge: crate::EdgeConfig, options: RunEdgeOptions) -> Result<(), EdgeError> {
    ...
    // is Some only when keepalive is enabled (inert under one-shot).
    ...
    let conf = pingora::server::configuration::ServerConf {
        ...
        // Set the proxy runtime thread count; the drain background service has its own thread.
        threads: edge_threads,
        // Set parallel accept tasks per listening socket, defaulting to one.
    ...
    // (pingora accept-loop has no idle timeout) — an unenforced limit, not set here.
    if let Some(app) = service.app_logic_mut() {
...
struct EdgeProxy {
    ...
    /// the per-request scratch pool (arena + frame buffer). One checkout at
    ...
    /// Boot-resolved native diagnostic route gate. This field exists only with
    /// `edge-bench` and enables `/edge-bench` only when OXO_EDGE_NATIVE_BENCH=1.
    #[cfg(feature = "edge-bench")]
    native_bench_enabled: bool,
    /// Optional busy-spin duration per request, resolved once from
    /// OXO_EDGE_INJECT_SPIN_US. Available only with `edge-bench`; it injects CPU
    /// work for instrumentation checks rather than adding a sleeping delay.
    #[cfg(feature = "edge-bench")]
...
impl EdgeProxy {
    fn new(edge: crate::EdgeConfig, options: EdgeProxyOptions) -> Self {
        ...
        let worker_socket_paths: Arc<[String]> = edge
            ...
            .collect();
        // One pool spans all worker sockets with an aggregate idle ceiling. The
        // default is max(512, workers * 32). Configuration resolution and descriptor
        // budget validation precede construction, so startup logs and /pool-health
        // describe the same actual pool.
        let worker_pool = Arc::new(frame_hop::WorkerFramePool::from_ceiling(
            frame_pool_idle.0,
            frame_pool_idle.1,
            unwrap_wait_budget,
        ));
        // Register the process-wide pool for admin health counters.
        ...
        Self {
            ...
            // (diagnostic): boot-time resolve of the native-floor flag. Read ONCE
            ...
            native_bench_enabled: std::env::var("OXO_EDGE_NATIVE_BENCH")
                .map(|v| v.trim() == "1")
                .unwrap_or(false),
            // Resolve CPU injection at boot and announce it when enabled.
            #[cfg(feature = "edge-bench")]
            inject_spin_us: {
    ...
    // Generate request IDs from a cached pid and monotonic sequence. Telemetry
    // reconstructs the same oxo-{pid}-{sequence} string, so all producers must keep
    // this format. Store the request string in the arena; its frozen reference can
    // cross awaits. Copy only when an HTTP or sidecar path needs ownership.
    ...
    async fn admit(
        ...
        // Direct-public identity uses the socket peer without reading forwarding
        // headers. Trusted-proxy identity uses already-lowered headers and parses
        // the peer before checking trust, preserving the rejection order.
...
/// Direct-public identity from the client address: absent means loopback,
/// an internet address yields its bare IP, and other address types use Display.
/// Tests compare this with the shared address-normalization function.
...
/// Connection-affinity key construction. H2 requests have no sticky key: their
/// concurrent streams must not all bind to one worker. Include the peer port to
/// distinguish connections from the same IP. Missing or epoch-zero timing also
/// returns no key rather than collapsing unrelated connections onto one pin.
...
// the sticky connection key's gates and traps, on the pure core.
#[cfg(test)]
mod sticky_key_tests {
    ...
    fn h2_is_gated_on_protocol_even_with_full_identity() {
        // pingora 0.8.1 hands H2 sessions a digest unconditionally, so the gate
    ...
    fn keys_differing_only_in_ephemeral_port_pin_independently() {
        // Connections from the same IP remain distinct because their ports enter the key.
    ...
    fn absent_or_degenerate_inputs_take_the_fallback_never_hash_defaults() {
        // no address, no timing, or an epoch-zero timestamp must all refuse —
...
// GUARD-HOLD CONTRACT: the two `*_admission` fields are RAII
...
struct EdgeRequestContext {
    ...
    // the sidecar (Ok(false)) path runs through Pingora's proxy loop, so both
...
impl ProxyHttp for EdgeProxy {
        ...
        Self::CTX: Send + Sync,
    {
        ...
        // one scratch checkout per request; the guard is a local owned by this
        ...
        // "ingest" seam: entry → collect → validate → admit → sanitize → body read
        ...
        // Static and diagnostic routes run after validation and admission, so header,
        // body, host and concurrency checks also cover them. Static fallthrough keeps
        // the same admission guard; crenel owns canonical path and response validation.
        // Its reusable result controls whether a completed static response can keep the
        // downstream connection alive.
        //
        // With edge-bench compiled and enabled at boot, the exact /edge-bench path
        // returns a native response through the shared frame-response writer. Query
        // strings do not affect that match. Optional CPU injection precedes both this
        // route and ordinary worker routing.
        #[cfg(feature = "edge-bench")]
        ...
        if self.native_bench_enabled && session.req_header().uri.path() == "/edge-bench" {
            // Use the fixed diagnostic body with the same downstream framing and keepalive policy.
        ...
        if let Some(static_server) = &self.static_server {
            ...
            // Production routing skips static lookup for unpinned first segments.
            // Without a pin set, GET/HEAD requests can discover new files immediately.
            // Percent-encoded segments stay candidates for canonical validation.
        ...
        // A1 / the frame hop sanitizes to a keep-PLAN over the collected
        ...
        // pins it with a full-frame byte-parity golden against the owned builder).
    ...
    async fn upstream_peer(
        ...
        // the per-request `group_key` bump and `idle_timeout = ZERO` below are two
        ...
        Self::CTX: Send + Sync,
    {
        if matches!(
            ...
            // do NOT mark the long-lived admission complete on the 101 upgrade —
    ...
    async fn logging(
        ...
        Self::CTX: Send + Sync,
    {
        // a long-lived sidecar connection (Action Cable or gRPC) is complete only
        ...
        Self::CTX: Send + Sync,
    {
        ...
        // only record a rejection if no terminal outcome was recorded yet. When a
...
mod prod_routing_tests {
    ...
    /// Self-cleaning temporary directory identified by pid and a process-local
    /// counter. Removing it on Drop keeps repeated filesystem tests isolated.
