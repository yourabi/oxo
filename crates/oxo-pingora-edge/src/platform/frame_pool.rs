//! Idle-connection pool with one process-wide ceiling and insertion order.
//! Checkout, eviction and closed-connection removal update all indexes under the
//! same lock. Pickup is FIFO within each worker key.
//!
//! After unlocking, checkout sends true through the pickup channel. Eviction
//! notifies the close waiter and drops both the pickup sender and pooled
//! connection. pop_closed is idempotent. No lock is held across notification,
//! connection destruction or await. Contention and lock-wait counters are exposed.
...
/// Worker key and connection ID used by checkout, idle watchers and eviction.
...
struct Inner<S> {
    ...
    /// Largest idle count observed immediately after insertion, before trimming.
    /// It can reach size + 1. That reading means the ceiling was reached, not that
    /// the uncapped working set is known.
    high_water: usize,
}
...
/// Default aggregate idle ceiling: max(512, workers * 32).
/// Each idle connection retains buffers and a file descriptor on the edge, plus
/// a descriptor and waiting thread or fiber on its worker. Idle timeout releases
/// these resources. Configuration resolution checks the ceiling against the
/// process descriptor budget; this function is pure and reads no environment.
pub(super) fn default_idle_ceiling(worker_count: usize) -> usize {
    512usize.max(worker_count.max(1) * 32)
}

/// Configuration source of the idle ceiling, exposed as frame_pool_idle_source.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
/// Pool implementation selector; unsupported names fail instead of selecting a fallback.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
pub(super) fn parse_pool_impl(raw: Option<&str>) -> Result<PoolImpl, String> {
    match raw.map(str::trim) {
        ...
        Some("pingora") => {
            Err("OXO_EDGE_POOL=pingora is unsupported; only owned is available".to_string())
        }
