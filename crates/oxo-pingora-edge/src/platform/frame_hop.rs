//! edge-side frame hop: pooled persistent UDS connections carrying the binary
//! `hop_frame` protocol, replacing the connect-per-request HTTP-text hop.
//!
//! ## Pool model (the TransportConnector way, on oxo's own pool since )
//!
//! The panel's finding #2 established that a bare `ConnectionPool<UnixStream>` cannot
//! detect a peer-closed idle connection (`idle_poll` needs an `OwnedMutexGuard`) and that
//! there is no pool-wide "evict all" API. So this mirrors `pingora-core`'s
//! `TransportConnector`: the pool stores `Arc<Mutex<UnixStream>>`, and each returned
//! connection gets a spawned WATCHER task that holds the lock and `select!`s over
//! (a) being picked up by `get()`, (b) eviction, (c) DRAIN, and (d) a 1-byte read
//! that detects peer-close/unexpected-data/idle-timeout — closing the connection on any
//! of the last three. `get()` waits on the same lock to know the watcher has released.
//!
//! the pool underneath is `frame_pool::ConnectionPool`, oxo-owned, with ONE
//! aggregate ceiling. pingora-pool's ceiling was per edge thread and left ghost entries
//! on cross-thread pickup (measured the effective ceiling as ~= the per-thread
//! flag); measured the owned pool at no cost against it and removed it.
//!
//! ## The two panel HIGH availability guards
//!
//! * **Stale-reuse retry** (finding #1): a pooled connection the worker closed a moment
//!   ago races the `get()`. If the FIRST write fails, or EOF arrives with ZERO response
//!   bytes, on a REUSED connection, the request was provably never dispatched (the worker
//!   dispatches only after reading a COMPLETE frame), so we retry ONCE on a fresh connect.
//!   Never-replay stays absolute for fresh connections and after any response byte.
//! * **Edge-closes-first** timeout ordering: the pool idle timeout
//!   ([`POOL_IDLE_TIMEOUT`]) is strictly less than the worker's connection lifetime, so
//!   the edge reaps an idle connection before the worker would, shrinking the race window.
//!
//! ## Put-back is frame-boundary-exact
//!
//! A connection is returned to the pool ONLY after a complete response exchange leaves it
//! at a frame boundary with an empty read buffer. Any leftover bytes, a mid-stream error,
//! a cancellation (the hop future dropped), or an active drain closes it instead — a torn
//! or ambiguous connection is never pooled.
...
/// edge always closes an idle connection first (the panel timeout-ordering invariant).
const POOL_IDLE_TIMEOUT: Duration = Duration::from_secs(15);

/// M2: a worker connection plus its persistent read scratch. The scratch makes the
/// response read SPECULATIVE — one `read_buf` typically grabs prefix+envelope together,
/// halving the steady-state data reads — and its emptiness at exchange end is the
/// machine-checked form of the put-back invariant ("an idle worker never speaks first",
/// so a pooled connection NEVER carries buffered bytes).
...
/// Speculative frame read buffer. Bytes live in `buf[pos..]`; frames are parsed out of
/// the front, socket reads append at the back via `read_buf` (which reads into SPARE
/// CAPACITY — no zero-fill; the panel #10 vetoed trading a ~150B alloc for a 16KiB
/// memset per request).
...
/// prefix declares (one growth + the kernel copy — documented in the mechanism ledger).
...
impl FrameReadBuffer {
    ...
    /// is processed WITHOUT another socket await (the panel #8 drain-boundary rule:
    ...
    async fn read_frame_or_drain(
        ...
        // prefix wait (as pre-); the envelope completion below is not interruptible.
        tokio::select! {
            biased;
...
pub(super) struct WorkerFramePool {
    ...
    /// B-1 shipped-build observables (panel MED-5: reuse correctness must be visible
    /// in SHIPPED builds — the bench-only gauges watch dispatch, not reuse).
    /// Checkouts served from the pool (reuse actually occurred).
    pub(super) pool_reuse_total: AtomicU64,
    /// `Arc::try_unwrap` failures at checkout that SURVIVED the bounded retry — the silent
    /// fresh-connect path. A rising rate here is the reuse-collapse tripwire (panel: an
    /// extra Arc clone makes this fail with green tests). Retried-and-resolved cases are a
    /// different phenomenon and are counted separately in `unwrap_retry_total`.
    ...
    /// B1 (graduation arc): successful fresh connects on a checkout miss. The M3 reuse
    /// gate is fresh/(fresh+reuse) ≤ 1% per arm — a breach means the pool is churning
    /// (idle-cap eviction storm or reuse collapse) and the arm re-registers, never scores.
    ...
    /// the ceiling this pool was built with, reported in the health snapshot so a
    /// cell's evidence carries the realized value and not only the flag it asked for.
    idle_ceiling: usize,
    ...
    /// B-1: single supervised reaper task per pool replaces per-checkin tokio::spawn.
    /// Lazily initialized on the FIRST checkin — the spawn context is therefore the same
    /// edge runtime that runs the hop (named per the lifecycle constraint).
...
/// B-1 checkin→reaper message: the guard + channels ONLY — never an extra Arc clone
/// of the connection (panel constraint: an extra clone makes `Arc::try_unwrap` fail
/// at checkout → silent fresh-connect with green tests).
struct WatchItem {
    meta: ConnectionMeta,
...
/// how long `checkout` waits for the watcher's in-flight `Arc` to go away before
/// declaring the extra reference PERSISTENT (see the comment at the retry site). Until
/// this was a count of 64 `yield_now`s, sized for a drop-order window "measured in
/// instructions"; it could not wait out a holder whose OS thread had been descheduled,
/// which is what a loaded box does, and it cost a pooled connection (a silent fresh
/// connect, `try_unwrap_fail_total`) each time. The budget is wall clock, set from the
/// measurement run: 58 transients in 200 runs of the unit suite at a 1 s budget
/// recovered in at most 3.9 ms (median 2.3 ms, the loop's own 64-yield pacing), so the
/// default is 20 ms, no less than three times the largest recovery observed
/// (docs/plans/-unwrap-budget.md). `Duration::ZERO` means exactly one attempt: the
/// pre-behaviour, kept as the same-binary control.
pub(super) const UNWRAP_WAIT_BUDGET_DEFAULT: Duration = Duration::from_millis(20);

/// The first attempts yield (a few hundred nanoseconds each; the common transient
/// resolves inside one), then the loop sleeps a millisecond between attempts so a
/// descheduled holder is waited for without spinning a core.
...
impl WorkerFramePool {
    /// The production constructor. The ceiling is ONE aggregate across the whole edge
    /// process (: the pool is oxo's own, `frame_pool.rs`; under pingora-pool it
    /// was per edge thread with ghost entries, which measured as "effective
    /// ceiling ~= the per-thread flag"). Since the value and its source arrive
    /// resolved from `platform::config::run_with_cli`, which reads
    /// `OXO_EDGE_FRAME_POOL_IDLE` once (default `frame_pool::default_idle_ceiling`,
    /// max(512, workers x 32), registered in ), validates `OXO_EDGE_POOL`,
    /// prints the posture line and checks the ceiling against the process descriptor
    /// limit (`crate::fd_budget`), all before the `--check-config` return. This pool
    /// reads no env, so the receipt on `/pool-health` cannot disagree with the notice.
    ...
    /// test-only: the default ceiling for `worker_count` workers, as production
    /// resolves it with the env unset (max(512, workers x 32)); the pre-constructor
    /// the older tests were written against, minus its env read.
    #[cfg(test)]
    ...
    /// B1: the pool-health snapshot as JSON — served on the private admin endpoint
    /// (`/pool-health`). Runtime exposure of the MED-5 shipped-build counters: the
    /// contract-2 test (warm-pool + once-only) and the M3 reuse gate both read this
    /// rather than inferring reuse from timing.
    pub(super) fn health_json(&self) -> String {
        // appended two keys, four more, one more, three more
        // (the unwrap budget and the wait it cost); existing keys and their
        // order are untouched (every consumer reads by key or greps a substring, none
        // pins the set). pool_impl is the A/B receipt; idle_high_water is the largest
        // idle count held, CLIPPED at the ceiling + 1 (a working set only when it reads
        // below the ceiling); the two lock counters are the contention observable;
        // frame_pool_idle_source says whether the ceiling was pinned or defaulted.
        format!(
    ...
    async fn checkout(&self, worker_idx: usize) -> Option<PooledConn> {
        ...
        // The lock being free does NOT imply the watcher's Arc is gone. tokio's
        // `OwnedMutexGuard::drop` releases the semaphore permit inside `Drop::drop`, and
        // its `lock: Arc<Mutex<T>>` field is only dropped AFTER that method returns — so a
        // checkout woken by the release can observe strong_count == 2 for the few
        // instructions in between (measured on a leak-free 8-thread probe loop: ~0.0025%
        // of checkins, matching the 0.0004-0.0055% seen in the sessions).
        //
        // That window is TRANSIENT: the other reference is already on its way out. A
        // genuine extra clone — the reuse-collapse defect this path's tripwire exists to
        // catch (panel) — is PERSISTENT and never resolves. separated the two by
        // retrying a bounded NUMBER of times; retries for a bounded TIME, because
        // the holder that is "on its way out" is a thread, and a thread can be
        // descheduled for far longer than 64 yields on a loaded box (the gate guest lost
        // a pooled connection this way in four runs). Within the budget the loop yields
        // first, then sleeps a millisecond at a time; at the budget it gives up exactly
        // as before, so `try_unwrap_fail_total` still counts the defect it was registered
        // for, and the wait itself is on `unwrap_wait_ns` / `unwrap_wait_max_ns`.
    ...
    fn checkin(&self, worker_idx: usize, conn: PooledConn, drain: watch::Receiver<bool>) {
        ...
        // B-1: OXO_HOP_REAPER=1 hands the watch future to the ONE supervised
        // reaper instead of spawning a task per checkin (the per-request spawn+drop +
        // cross-thread waker churn was the -measured futex/epoll cost). The default
        // stays the per-checkin spawn: the M3 T2 ship-what-you-scored rule demoted the
        // reaper to an opt-in flag (LO+reaper lost p99 vs LO-alone; see reaper_enabled).
        // The unset path doubles as the same-binary A/B control (B3 pattern).
        let use_reaper = {
            #[cfg(test)]
            {
        ...
        if let Err(send_err) = tx.send(WatchItem {
            ...
            // Reaper channel closed (should not happen while supervised): pop the
            // connection out of the pool rather than leaving it in watcher-less
            // (W3.5, panel HIGH-1c — the pre-comment leaned on the checkout
            // MSG_PEEK probe as the safety leg for this state; with the probe folded
            // into reactor-cached readiness, an unwatched idle conn must not linger).
            // The returned WatchItem — guard included — drops after the pop, releasing
            // the mutex; the counter above keeps the state visible.
            let item = send_err.0;
...
/// Boot-time reaper toggle (read once). Default OFF — the M3 T2 session's
/// pre-committed ship-what-you-scored rule fired: LO+reaper (arm 2c) lost the p99 axis
/// to LO-alone (arm 2: 7.273 ms vs 7.621 ms) and fell to 3/4 vs bare puma, while
/// LO-alone swept 4/4 vs both baselines. `OXO_HOP_REAPER=1` opts in (directionally
/// positive on CPU at T1, sub-MDE; documented flag, not the default).
fn reaper_enabled() -> bool {
    // Test-only force: the reaper battery must exercise the reaper path regardless of
    // the shipped default (env + OnceLock are process-wide and race across #[test]s).
    #[cfg(test)]
...
/// B-1: supervisor for the per-pool reaper. Respawns the reaper on panic (a dead
...
/// B1: register the production pool for admin `/pool-health` reads. One edge process
...
/// B-1: the consolidated reaper — one task drives ALL idle-connection watch futures
...
/// Non-blocking liveness probe at checkout: an idle worker never speaks first, so any
/// readable byte (data or EOF) means the connection is NOT reusable.
///
/// W3.5 (panel HIGH-1 compliant fold): the pre-probe issued recv(MSG_PEEK)
/// — a real syscall on EVERY pooled checkout (measured exactly 1.00/request on the
/// keepalive ledger). `try_read` gives the SAME three-way classification (WouldBlock =
/// live, Ok(0) = peer closed, Ok(>0) = dirty) but consults tokio's reactor-cached
/// readiness first: the clean steady state (no cached readiness) returns WouldBlock with
/// ZERO syscalls, and the read syscall fires only when the fd already signalled — the
/// exact suspicious case the probe existed to catch. Coverage of the three panel cases
/// is unchanged: (a) stray pre-dispatch bytes and (b) half-close both raise epoll events
/// that the reactor records on the ScheduledIo whether or not any task is polling, so
/// they are visible here before the write and still map to drop-and-retry; (c) a
/// watcher-less conn (reaper send-failure) is covered the same way. try_read CONSUMES a
/// dirty byte where MSG_PEEK did not — irrelevant, because both paths drop the
/// connection on that classification. The race window (bytes arriving between this check
/// and the write) is identical to the old probe's probe-to-write window.
...
/// Build the pre-parsed request frame from the session, the trusted hop metadata, the
/// sanitized app headers, and the buffered body. The trusted fields (remote_addr,
/// scheme, server name/port) become NATIVE frame fields; the x-oxo-* headers the
/// HTTP hop would have carried are excluded (the worker drops them anyway — this keeps
/// the resulting Rack env byte-identical to the HTTP hop: the env-parity guarantee).
/// W3d: the shipped frame path — request bytes straight from borrows (the sanitize
/// plan's keep-mask over the collected headers), no intermediate survivor Vec, no owned
/// RequestFrame. The owned builder below STAYS as the byte-parity oracle and the
/// non-frame consumers' path.
...
/// Session-free core so the byte-parity golden can drive the REAL shipped path against
/// the owned oracle with identical inputs.
#[allow(clippy::too_many_arguments)]
fn assemble_request_frame_bytes_into(
    ...
    // bench-only, rides the hop-timing feature); CLOCK_MONOTONIC nanos at frame build,
...
/// Test-facing twin returning a fresh Vec — keeps the byte-parity golden's call shape
/// (and proves the `_into` form against the historical bytes).
#[cfg(test)]
#[allow(clippy::too_many_arguments)]
...
// W3: no longer on the shipped path — it is the byte-parity ORACLE: the session
// inputs are passed as plain parts (exactly what the shipped wrapper extracts), and the
// body below is the pre-W3 owned pipeline VERBATIM. The golden asserts the new borrow
// pipeline produces byte-identical frames against this. Do not delete without replacing
// the oracle.
#[cfg(test)]
fn assemble_request_frame_owned(
    ...
    // The metadata is carried in native fields; never also as x-oxo-* headers.
    // A1: the bare sanitize no longer attaches metadata headers and the sanitize
    // core already strips client-sent x-oxo-* (should_strip_header), so in the
    // shipped path this retain removes nothing — it is an allocation-free
...
/// B1/B3 → M4: per-worker in-flight gauges + the dispatch-mode selector.
/// SHIPPED since (panel MED-9 promotion): least-outstanding dispatch is the default and
/// needs the in-flight gauges + candidate order in every build. The TELEMETRY layer
/// (dispatch_total/chosen_busy/bad_dispatch counters, depth histogram, /hop-timing report)
/// stays bench-only behind `hop-timing`.
///
/// Gauge integrity is the panel-HIGH invariant: the gauge is held by an RAII guard scoped to
/// ONE worker attempt — created only once an attempt on an idx is committed (post-checkout,
/// pre-write) and decremented on Drop, which covers success, AfterWrite errors, the
/// stale-retry re-exchange (same attempt, same idx, guard simply stays held), and
/// hop-future cancellation (client disconnect drops the future → Drop runs). A
/// Connect-failover to the next idx never created a guard for the failed idx.
pub(super) mod dispatch {
    ...
    pub struct Gauges {
        /// SHIPPED: the least-outstanding selector's input.
    ...
    /// Boot-time dispatch mode (env twin; read once). M0's sim killed the LO-above-K
    /// hybrids (degenerate at c=16) and carried exactly one: free-first.
    ...
    pub enum DispatchMode {
        ...
        /// preserving RR's rotation pattern (the p50-friendly half of the trade).
        FreeFirst,
    ...
    pub fn mode() -> DispatchMode {
        *MODE.get_or_init(|| match std::env::var("OXO_WORKER_DISPATCH").as_deref() {
            ...
            // M4 default flip (T2-gated): least-outstanding is the shipped default —
            // the causal result (p99 −36%, first full-scoreboard win vs bare puma).
    ...
    // ---- : the per-worker admission cap ("dispatch-on-free") -----------------------
    //
    // proved the tail is ordinary requests bunching in per-worker queues (latency
    // +~1.5 ms per committed request ahead). The cap bounds committed requests per
    // worker at K; overflow PARKS in one shared FIFO here at the edge and re-runs
    // worker selection on wake (late binding). Default OFF: `cap()` returns None and
    // every path below is byte-identical to the pre-behavior.
    //
    // MEMORY-ORDERING PROTOCOL (panel C4 — the no-lost-wakeup proof):
    // Writer (a completing request), in program order:
    //     D1: in_flight[idx].fetch_sub(1, SeqCst-when-capped)
    //     D2: if PARKED.load(SeqCst) > 0 { free_slot().notify_one() }
    // Waiter (a parking request), in program order:
    //     W1: PARKED.fetch_add(1, SeqCst)
    //     W2: create + enable the Notified future
    //     W3: re-scan in_flight[i] (SeqCst loads, ONLY over the at-capacity set)
    //     W4: await the Notified future
    // If the writer's D1 lands before the waiter's W3, the waiter sees the freed slot
    // and never sleeps. If D1 lands after W3, then W1 (SeqCst, before W3) is visible to
    // the writer's D2 (SeqCst), so notify_one() fires — and W2 ran before W4, so the
    // enabled future stores the permit even if notify_one raced ahead of W4. Either
    // way: no lost wakeup. All four seams are SeqCst; nothing else changes ordering.
    // (On x86 the SeqCst RMWs compile to the same lock-prefixed instructions, so the
    // uncapped path's cost claim is unchanged; the D1 upgrade applies only when the
    // cap is enabled.)

    /// Requests currently parked (shipped gauge; /ready + wake gating).
    pub static PARKED: AtomicUsize = AtomicUsize::new(0);
    /// Lifetime parked-event counter (shipped; the engagement receipt for scored
    /// timing-off cells, panel C7).
    ...
    /// the Drop-side wake gate. Latched true whenever a cap is resolved (boot or
    /// env) so the completion path pays exactly one Relaxed bool load when uncapped —
    /// stated honestly: NOT byte-identical to pre-, but unmeasurable. Tests latch
    /// it directly (the cap OnceLock is first-writer-wins process-wide, panel C6).
    ...
    /// mirroring `mode()`. None = uncapped (the shipped default).
    ...
    impl ParkGuard {
        pub fn enter() -> ParkGuard {
            PARKED.fetch_add(1, Ordering::SeqCst); // W1
    ...
    /// Any worker in `mask` (the at-capacity set from the failed pass, panel C1) now
    /// below `k`? SeqCst loads — the W3 seam; never reuse the Relaxed depth helper.
    ...
    // ---- : sticky per-connection worker affinity --------------------------------
    //
    // The ledger proved the tail is per-worker bunching from per-request worker
    // re-picking; Falcon's source (addendum) showed the missing ingredient: pin
    // each downstream connection to one worker so a request only waits behind its own
    // connection's previous request. Panel-fixed invariants: assignment is by FEWEST
    // PINS, not momentary in-flight depth (an idle-but-pinned worker must not collect
    // every new pin, P2); a pin moves only on a Connect failover, never on AtCapacity
    // (the cap must not silently herd pins, P8); unkeyed requests are their own
    // counted path (P6). Default OFF: mode() must be Sticky or none of this runs.

    /// Shipped engagement receipts (unconditional — the scored cells read these).
    pub static STICKY_HIT_TOTAL: AtomicU64 = AtomicU64::new(0);
    pub static STICKY_MISS_TOTAL: AtomicU64 = AtomicU64::new(0);
    /// Requests that could not be keyed (H2, absent addr/timing): dispatched
    /// least-outstanding and counted HERE — never as hit or miss — so a silently
    /// unkeyed cell is visible (P6).
    pub static STICKY_FALLBACK_TOTAL: AtomicU64 = AtomicU64::new(0);
    pub static STICKY_EVICT_TOTAL: AtomicU64 = AtomicU64::new(0);

    /// Per-worker pinned-connection counts: the assignment key (P2) and the
    /// realized-assignment receipt in /ready.
    pub static PIN_COUNTS: [AtomicUsize; MAX_WORKERS] =
        [const { AtomicUsize::new(0) }; MAX_WORKERS];

    const STICKY_SHARDS: usize = 16;
    /// Hard cap on stored pins across all shards; at the per-shard share the whole
    /// shard clears (crude, rare, self-healing — cleared connections re-pin balanced
    /// on their next request). The bench holds 16 entries.
    ...
    /// Park-wake re-read: the pin may have moved while this request parked. A
    /// cleared entry restores the prior pin (keeping the distinct-entry receipt
    /// truthful) rather than re-running assignment.
    ...
    /// Connect-failover re-pin (P8: Connect ONLY, never AtCapacity): the pinned
    /// worker was actually unreachable and `to` served this request, so the pin
    /// follows. No-op if the stored pin already moved.
    ...
    /// Pin-count snapshot for /ready (the realized-assignment receipt: expected
    /// 4/4/4/4 at c=16 over 4 workers).
    ...
    /// Distinct stored pins (receipt: == the client connection count in a clean cell).
    ...
    impl InFlightGuard {
        /// Record the dispatch decision + take the gauge. Call once an attempt on `idx` is
        /// committed. The snapshot across gauges is non-atomic (stated in the plan; the
        /// counters are indicative, the A/B verdict comes from the gate, not these).
        /// Test-facing uncapped form (production goes through try_acquire).
        #[cfg(all(test, feature = "hop-timing"))]
        ...
        /// the capped commit. `k = None` is the shipped uncapped path (plain
        /// fetch_add, Relaxed — byte-identical to the pre-acquire). With a cap,
        /// the commit is a CAS that succeeds only below K (SeqCst — the proof seam);
        /// a raced loss returns None and the caller returns its UNTOUCHED connection
        /// to the pool (zero bytes written, zero-replay unaffected).
        /// Ledger writes happen ONLY on success, exactly once per admitted request
        /// (panel C8 — capped cells stay comparable to the banked baseline).
        pub fn try_acquire(
            idx: usize,
            worker_count: usize,
            k: Option<usize>,
        ) -> Option<InFlightGuard> {
            // >16-worker configs degrade to shared-slot counting rather than panicking;
            // read_worker_dispatch refuses least-outstanding beyond MAX_WORKERS at boot
            // (M4, panel MED-9), so the clamp is unreachable under the shipped default.
    ...
    impl Drop for InFlightGuard {
        fn drop(&mut self) {
            // D1/D2: with the cap enabled the decrement is the proof's writer seam
            // (SeqCst) and every completion offers one wake to the parked FIFO. Uncapped
            // keeps the original Relaxed decrement — byte-identical to pre-.
            if capped() {
...
/// W0: the worker KIND behind this edge — classic S1 binaries or async fiber
/// reactors. Gates the stale-reuse retry carve-out below: for a classic worker,
/// zero-response-byte EOF on a reused connection soundly means "never dispatched"
/// (single request per connection, dispatch is immediate); for a fiber reactor the same
/// signature is ambiguous under crash/drain-abandonment (GRADUATION_DESIGN §3c), so the
/// retry — and the sibling failover its failed reconnect would escape into — must not
/// run: a possibly non-idempotent request would be silently replayed.
pub mod worker_kind {
    use std::sync::OnceLock;
...
pub(super) async fn send_worker_frame_to_pool(
    ...
    let mut woken = false;
    // barging guard (panel C2): with the cap on, a fresh arrival that finds anyone
    ...
    loop {
        ...
        // M4: dispatch mode is SHIPPED — least-outstanding (ascending in-flight, RR
        // tie-break) is the default; rr and free-first (bench-tier, measured worse)
        // remain selectable via --worker-dispatch / OXO_WORKER_DISPATCH.
        // a pinned request puts its worker first (failover tail unchanged); a
        // park-wake pass re-reads the pin, which may have moved while parked.
        ...
        // (panel C1): track WHICH workers reported at-capacity this pass. A parked
        ...
        // (P8): only a Connect failure ON THE PINNED worker licenses a re-pin to
...
/// wait until any worker in `mask` drops below `k`, or drain begins (503).
/// Enable-then-recheck on the shared Notify (the W1-W4 protocol; proof comment in the
/// dispatch module); the ParkGuard decrements the parked gauge on EVERY exit including
/// cancellation of this future. mask == u32::MAX is the barging-guard form: any slot.
async fn park_for_slot(mask: u32, k: usize, drain: &watch::Receiver<bool>) -> Result<(), u16> {
    ...
    let _parked = dispatch::ParkGuard::enter(); // W1 (+ the shipped park_total counter)
    let mut drain_rx = drain.clone();
    loop {
        let notified = dispatch::free_slot().notified();
        tokio::pin!(notified);
        notified.as_mut().enable(); // W2: registered before the re-scan — no lost wakeup
        if *drain_rx.borrow() {
            return Err(503); // drain re-check AFTER registration and on every wake
        }
        if dispatch::any_admittable(mask, k) {
            // W3 saw a free slot. We consumed (or will consume) a permit that might
            // have been meant for another waiter — forward one wake so a sibling
            // parked on the same event is not stranded (panel C2's lost-race repair).
        ...
        tokio::select! {
            ...
            _ = notified.as_mut() => {} // W4: woken — loop back to the re-scan
        }
    }
}
...
async fn send_to_worker(
    ...
    // M-C seam: "checkout" = obtain a worker connection (pool hit, or fresh connect on miss).
    #[cfg(feature = "hop-timing")]
    ...
    let (mut conn, reused) = match pool.checkout(worker_idx).await {
        ...
        None => {
            ...
            // B1: counted AFTER connect succeeds — a refused/dead socket is a failover
            // (Connect error above), not a fresh connect the reuse gate should see.
    ...
    // B1: the attempt on this idx is now COMMITTED (a connection exists; the write is
    // next). The RAII guard covers every exit from here — success, AfterWrite, the
    // stale-retry re-exchange below (same attempt, same idx), and future-drop cancellation.
    // A Connect failure above returns before this line, so failover never counts a dead idx.
    // M4: SHIPPED (the least-outstanding default reads these gauges in every build).
    // the commit is now a capped CAS. A raced loss (another request took the last
    // slot between the pre-check and here) returns the UNTOUCHED connection to the pool
    // — zero bytes written, zero-replay unaffected — and reports at-capacity.
    ...
    .await
    {
        Ok((status, reusable)) => {
            ...
            if reusable && !*drain.borrow() {
                // M-C seam: "checkin" = return the connection to the pool at a clean boundary.
                #[cfg(feature = "hop-timing")]
        ...
        {
            // Panel HIGH #1: a reused connection failed its FIRST write or hit EOF with
            // ZERO response bytes — the worker never dispatched, so this is safe to retry
            // exactly once on a fresh connect.
            //
            // W0: the DELIVERED-then-EOF signature is CLASSIC-ONLY. A fiber
            // reactor's crash/drain closes in-flight connections with exactly that
            // signature while the request MAY have dispatched — and a failed reconnect
            // here would escape as FrameSendError::Connect into the sibling-failover
            // loop, silently replaying a committed, possibly non-idempotent request.
            // Under async kind that case skips this arm and the generic arm below
            // returns AfterWrite(502): terminal, no retry, no failover, zero replays
            // (the §3c retry-disable, delivered early). The WRITE-FAILED signature
            // (request_delivered=false — nothing reached the worker) stays
            // retry-eligible for BOTH kinds: a corpse pooled conn after a reactor
            // crash must fail over to a live sibling, not fail the request.
            ...
            // B1: the stale-retry reconnect is also a fresh connect the reuse gate must
            // see (a stale storm and an eviction storm both surface as pool churn).
            pool.fresh_connect_total.fetch_add(1, Ordering::Relaxed);
            match exchange(
                &mut fresh,
                request_bytes,
...
enum ExchangeError {
    /// First write failed or EOF-before-any-response-byte — retry-eligible on reuse.
    /// W0: `request_delivered` splits the two stale signatures. `false` = the write
    /// itself failed (nothing reached the worker; a truncated partial is the worker's
    /// contract-1b decode-error close, still undispatched) — retrying is safe for EVERY
    /// worker kind. `true` = the write succeeded and the read hit zero-byte EOF — for a
    /// classic worker that still proves never-dispatched, but for a fiber reactor it is
    /// crash/abandonment-ambiguous and must be terminal (the §3c zero-replay contract).
...
// inside one depth. Unused without the hop-timing feature (bench-only recording).
#[allow(clippy::too_many_arguments)]
async fn exchange(
    conn: &mut PooledConn,
    request_bytes: &[u8],
    ...
    // M-C seam: "request_write" = write_all of the request frame to the worker socket.
    #[cfg(feature = "hop-timing")]
    ...
    // Read the first response frame. EOF with zero bytes here = stale-before-dispatch.
    // (A pooled connection always starts an exchange with an EMPTY buffer — the
    // checkin surplus guard enforces it — so "zero bytes available" is well-defined.)
    // M-C seam: "response_read" = wait+read of the first response frame. THIS is where
    // the off-CPU hop wait (worker dispatch + Ruby round-trip) that task-clock is blind to lands.
    #[cfg(feature = "hop-timing")]
...
/// Build a fixed-length downstream response header from a decoded Full frame. Mirrors the
/// HTTP hop's `parse_worker_response` EXACTLY: the framing headers are dropped, then
/// `Content-Length` is set explicitly from the body length (pingora does NOT auto-frame it).
/// keepalive-aware, mirroring the HTTP hop (worker_wire.rs:718-725) — an explicit
/// `Connection: close` is inserted ONLY under one-shot; under keepalive it is omitted so
/// pingora emits the correct `Connection` header from `set_keepalive`. NOTE (re-baseline,
/// guest-run-20260713-233…): the prior unconditional `Connection: close` was REDUNDANT, not a
/// regression — `set_keepalive(Some)` already governed downstream reuse, so was already
/// keepaliving (post-fix 957µs ≈ 947µs on /bench). This change is correctness/clarity
/// (drop a self-contradictory header, match the audited HTTP hop), NOT a measured perf win.
/// M2 (bench-only): write a canned native response reusing the EXACT framing the frame
/// hop's Full-response path uses — `build_response_header` (framing-header drop + explicit
/// Content-Length + keepalive-aware Connection) followed by the same timeout-guarded header
/// and body writers. The `/edge-bench` native-floor stub calls THIS rather than reaching
/// into the private writers, so the on-wire framing is byte-identical to a `/bench` Full
/// response and the write path (and thus its CPU cost) is the same. Compiled only under the
/// `edge-bench` feature; absent from the shipped binary.
...
fn build_response_header(
    ...
    // W1: headers arrive OWNED from decode_response and die right after this call —
    // move them into the ResponseHeader instead of cloning every name+value.
    // (audit item 9): capacity for OUR inserts (Content-Length / Connection) PLUS
    // the headers pingora injects downstream (Date, and Connection under keepalive).
    // The old +2 was exactly full after our own inserts, so pingora's Date insert grew
    // the HeaderMap — 4 re-allocations (indices + entries in the value map AND the
    // case map) on EVERY response.
...
fn is_framing_header(name: &str) -> bool {
    // W1: case-insensitive compare without the per-header String that
...
mod tests {
    ...
    /// pipeline (sanitize_frame_headers → assemble_request_frame_bytes) over an
    /// adversarial corpus. The worker builds its Rack env in frame order, so byte
    /// equality here IS env parity. Any intentional wire change must update BOTH
    /// pipelines and this comment.
    #[test]
    fn frame_bytes_parity_old_vs_new_pipeline() {
        ...
        for case in &cases {
            ...
            // NEW pipeline (the shipped path's session-free core).
    ...
    // M0: the frame hop is keepalive-aware like the HTTP hop — an explicit
    // `Connection: close` is written ONLY under one-shot; under keepalive it is omitted so
    // pingora emits the correct Connection header from set_keepalive. (The prior unconditional
    // close was redundant, not a regression: set_keepalive already governed reuse — the
    // re-baseline confirmed post-fix ≈ pre-fix on /bench. Correctness/clarity, no perf change.)
    #[test]
    ...
    // Liveness probe: a fresh idle connection is live; a peer-closed or data-bearing one
    // is not (an idle worker never speaks first, so any readable byte = not reusable).
    // W3.5: classification is via reactor-cached readiness (try_read), so a state
    // change becomes visible once the reactor has observed the epoll event — the polls
    // below model that (production timing: reactor-poll latency, µs-scale; a raced miss
    // falls into the write-EPIPE retry / delivered-then-EOF taxonomy, same as a miss in
    // the old probe-to-write window).
    ...
    // B1: the /pool-health JSON shape is a parsing contract — the M3 reuse gate and the
    // contract-2 test both read these exact keys. A reuse increments pool_reuse_total
    ...
    // A4.5 root-cause probe (2026-08-03): the reuse-collapse tripwire fired 1-13 times per
    // ~236k checkins in the banked sessions. Hypothesis: tokio's `OwnedMutexGuard::drop`
    // releases the semaphore permit BEFORE its `lock: Arc<Mutex<T>>` field is dropped, so a
    // checkout woken by that release can run `Arc::try_unwrap` on another core while the
    // strong count is still 2. That predicts the failure is TRANSIENT (resolves in
    // nanoseconds) — a leaked clone, the defect the tripwire exists for, would be PERSISTENT.
    // This probe drives the handshake hard and reports the rate; it asserts nothing about the
    // count (it is a race), only that the loop actually exercised reuse.
    ...
    async fn evictions_are_exactly_n_minus_c(force_path: u8) {
        ...
        // the test constructor pins the ceiling, so the receipt names the env
    ...
    /// the ceiling is AGGREGATE. Four checkins at ceiling 1, each on its own OS
    /// thread (a barrier keeps four blocking-pool threads alive at once, so no thread is
    /// reused), evict exactly three; one survives. Under pingora-pool this test asserted
    /// the opposite (all four survived: one cache per thread), which is the defect.
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    ...
    /// a cross-thread pickup leaves no ghost. Check in four on one OS thread, pick
    /// two up from the runtime thread, check in two more from a third thread (idle is
    /// back at the ceiling, no eviction), then one more evicts exactly one. Under
    /// pingora-pool the first thread's cache held two ghosts after the pickups and the
    /// later checkins evicted live entries early.
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    ...
    /// The never-replay boundary (panel #9): EOF after a PARTIAL prefix is Err(502) —
    ...
    /// served from the buffer WITHOUT another socket await (panel #8 — proven by never
...
/// B1 (panel HIGH #2): the gauge-integrity tests — the in-flight gauge must return to
/// zero on EVERY exit path, or B1's counters corrupt and B3's least-outstanding picks bias
/// away from the leaked worker. Tests use dedicated high worker indices (10-13) so the shared
/// process-global gauges don't collide with each other under the parallel test runner, and
/// assert RELATIVE deltas on the global counters.
#[cfg(all(test, feature = "hop-timing"))]
mod dispatch_tests {
    use super::dispatch::*;
    ...
    fn guard_returns_to_zero_on_future_cancellation() {
        ...
        rt.block_on(async {
            ...
            assert_eq!(
                g.in_flight[idx].load(Ordering::Relaxed),
                0,
                "cancellation must release the gauge (panel HIGH #2)"
            );
        });
    }
    ...
    // ---- : the dispatch-on-free battery (panel-corrected design) -----------------
    // Every test that touches the shared gauges/PARKED takes gauge_test_lock (the
    // no-concurrent-gauge-tests rule extends to the park statics, panel C6).
    ...
    fn capped_hammer_never_exceeds_k_and_ledger_counts_admits_only() {
        ...
        // dispatch_total must advance exactly once per ADMITTED request (panel C8).
        let _serial = gauge_test_lock();
        let idx = 14;
        #[cfg(feature = "hop-timing")]
    ...
    fn failed_cas_writes_nothing_to_the_ledger() {
        // Panel C8, deterministic form: failed try_acquire attempts must leave the
        // ledger untouched; successes write exactly once.
        let _serial = gauge_test_lock();
        let idx = 5;
    ...
    async fn drop_wakes_a_parked_waiter_from_another_thread() {
        // The SeqCst W1-W4/D1-D2 protocol under genuine parallelism (panel C5): a
        // waiter parks on a full worker; a completion on another OS thread frees the
        // slot; the waiter must admit. Latch the wake gate first.
        let _serial = gauge_test_lock();
    ...
    async fn dead_worker_never_unparks_a_waiter() {
        // Panel C1: a connect-dead worker (gauge stuck at 0) is NOT in the at-capacity
    ...
    async fn cancelled_waiter_forwards_the_wake() {
        // Panel C2 repair: waiter A parks, gets cancelled; the completion wake must
        // still reach waiter B (Notify forwards an unconsumed permit).
        let _serial = gauge_test_lock();
        set_capped_for_tests(true);
        let idx = 11;
        let g1 = InFlightGuard::try_acquire(idx, 16, Some(1)).expect("fill");
...
// the sticky-affinity battery. Plain cfg(test) -- the pin store, counters, and
// assignment run in every build (the receipts are shipped), unlike the hop-timing
// dispatch battery above. Every test takes the shared serial lock and resets the
// store: the shards, pin counts, and gauges are process-global singletons.
#[cfg(test)]
mod sticky_tests {
    use super::dispatch::*;
    use std::sync::atomic::Ordering;
    ...
    fn sequential_16_misses_land_4_4_4_4() {
        // The bench shape (P2): 16 connections, 4 workers, zero gauges -- the
    ...
    fn concurrent_misses_stay_balanced() {
        // P2: N threads pinning distinct keys concurrently -- the assign lock makes
    ...
    fn repin_moves_on_connect_and_ignores_stale_from() {
        ...
        assert_eq!(idx, 0);
        // The Connect-failover re-pin (P8): counts follow the pin.
    ...
    fn fallback_counts_its_own_lane() {
        // P6: unkeyed requests are neither hits nor misses.
        let _serial = fresh();
    ...
    fn at_capacity_pinned_worker_never_repins_p8() {
        // P8, on the real primitives: the pinned worker is full under the cap
        // (try_acquire returns None -> the loop reports AtCapacity and moves on).
        // The request serves elsewhere, and because only a Connect failure licenses
        // sticky_repin, the stored pin MUST NOT move -- a briefly full worker under
        // the cap must not herd pins (that would be the deferred escape-hatch
        // variant arriving unmeasured).
        let _serial = fresh();
