...
pub(super) struct SlowClientPolicy {
    ...
    /// N minus 1). `Some` only when keepalive is enabled; `None` under one-shot
...
/// H2 connection idle-timeout and absolute-age are framework-blocked (unenforced limit).
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
impl SlowClientPolicy {
    ...
    /// floor at 1. This is the "no path yields Some(0)" guarantee.
...
impl IdentityFairnessLimiter {
    ...
    pub(super) fn acquire(
        ...
        let mut active = self
            ...
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
        // when the identity is already tracked, clone the EXISTING map key
...
mod tests {
    ...
    fn keepalive_idle_secs_rounds_up_and_floors_at_one() {
        // the ms->s conversion must never yield 0 (Some(0)=Infinite=slowloris).
