...
fn run_with_cli(cli: crate::EdgeCliConfig) -> Result<(), EdgeError> {
    ...
    // Resolve the serve-rails preset once and pass it to all consumers so
    // keepalive, SSE, static mounts, startup logs and /ready agree. Validate
    // the app root even when an explicit static preset overrides its mounts.
    ...
    // worker kind gates the frame hop's stale-reuse retry (async reactors make
    ...
    // posture line: the resolved composite is stated where the operator
    ...
    // the same reason (the pool reads no env; the value cannot disagree).
    ...
    // worker). Report the ceiling and unwrap wait budget together at startup.
    eprintln!(
    ...
    if cli.check_config {
        // the docroot pin audit normally lives in run_edge, AFTER this
...
/// the single serve-rails resolution. Computed ONCE in `run_with_cli`
...
pub(super) struct ServeRailsPlan {
    ...
    /// or a pre-`assets:precompile` checkout must not be refused boot.
    static_assets: bool,
...
/// /ready posture snapshot — threaded into telemetry so the resolved
...
/// derivation matrix. The ROOT check is UNCONDITIONAL — the root
/// is the posture anchor even when `--static-rails-preset` overrides the mounts, so a
/// typo'd root can never silently degrade. Checks are symlink-following `is_dir`
/// Capistrano `current` symlinks resolve; a dangling symlink reads as missing.
...
fn plan_serve_rails(
    ...
    if !static_assets {
        // API-only apps and pre-precompile checkouts have public/ but no
...
/// errors, and the serve-rails plan contributes its derived mounts here.
fn read_static_mounts(
    ...
    // Log the resolved mount table. A `/` fallthrough mount matches every path,
    // so the table explains which requests are candidates for static serving.
    if raw_specs.is_empty() {
...
/// Resolved proxy-service thread count and its configuration source. Retain
/// the available processor count so oversubscription can be reported.
#[derive(Debug)]
...
/// Upper bound for parallel accept tasks per listening socket. Each is a
/// long-lived Tokio task, so reject settings that could spawn an unbounded set.
const MAX_LISTENER_TASKS: u64 = 64;

/// Parallel accept tasks per listening socket, defaulting to one. This controls
/// accept concurrency within the process. SO_REUSEPORT process fan-out remains
/// unsupported because it would divide process-local security counters.
...
/// Resolve edge runtime threads. Use one thread on hosts with at most two
/// available processors, otherwise the available processor count. Explicit values
/// above that count are allowed up to the cap and reported at startup.
fn read_edge_threads(cli: &crate::EdgeCliConfig) -> Result<EdgeThreadsConfig, EdgeError> {
    ...
    let nproc = std::thread::available_parallelism()
        ...
        .unwrap_or(1);
    match value {
        // Use one edge thread on small hosts; otherwise use the available processor count.
...
/// Resolve the hop wire format, defaulting to pooled binary frames. Frame lengths
/// are u32; reject larger body caps rather than truncating their wire representation.
fn read_worker_hop(
    cli: &crate::EdgeCliConfig,
    max_body_bytes: u64,
...
/// the worker kind behind this edge (env-only — the service supervisor forwards
...
/// the dispatch-discipline ladder (flag > env > default least-outstanding).
/// Fail-closed guard: the in-flight gauges have MAX_WORKERS=16 slots; a
...
fn read_public_mode_gate(
    ...
    if bind.ip().is_loopback() {
        // Public identity settings require a non-loopback bind. Reject them on a
        // loopback listener instead of silently applying direct-peer semantics.
        // TLS, server-name checks, fairness and long-lived limits also apply to
        // loopback listeners and are not part of this refusal set.
        let public_mode_set =
    ...
    for (knob, effect) in render_unenforced_knobs(slow_client) {
        eprintln!(
            "oxo_edge_config_warning NOT ENFORCED: {knob} is parsed and reported but has no \
             pingora 0.8.1 enforcement seam; it does not bound {effect} (see docs/THREAT_MODEL.md)."
        );
    }
}
...
mod tests {
    ...
    fn edge_threads_resolver_defaults_precedence_and_guards() {
        ...
        // Defaults use one edge thread on hosts with at most two available processors, otherwise nproc.
        if nproc <= 2 {
    ...
    fn worker_dispatch_resolver_defaults_precedence_and_guards() {
        // one test for every scenario (env vars are process-global; parallel tests).
        ...
        // depth-reading modes refuse fleets beyond the gauge slot count;
    ...
    fn render_unenforced_knobs_names_only_set_inert_knobs() {
        // a stock deployment (defaults) emits nothing — no noise.
    ...
    /// The Rails preset includes a `/` fallthrough mount, which makes every path a
    /// static candidate at the mount-matching layer. This checks configuration shape;
    /// it does not count filesystem calls or override the separate production routing
    /// filter in the proxy.
    #[test]
    fn serve_rails_derives_a_root_fallthrough_mount_making_every_request_a_static_candidate() {
        ...
        // The derived configuration always includes a `/` fallthrough mount.
        let root = mounts
            .iter()
            .find(|spec| spec.prefix == "/")
            .expect("serve-rails must derive a `/` fallthrough mount");
        assert_eq!(
            root.docroot,
    ...
    fn resolve_serve_rails_cli_wins_env_twin_empty_is_unset() {
        // the single resolver, unit-tested with injectable getenv — no
    ...
    fn serve_rails_posture_ladder_explicit_cli_beats_preset() {
        // The preset enables keepalive and SSE. Explicit disabling flags override it;
        // fake_upstream separately checks the environment override path.
    ...
    fn plan_serve_rails_directory_shape_matrix() {
        // unit coverage of the four shapes (integration re-proves them on a
        ...
        // Shape 1: missing root -> fail-closed refusal naming --serve-rails, even
        ...
        // Shape 3: public/ is a regular FILE -> broken layout, fail-closed.
        ...
        // A public directory without assets derives only the fallthrough mount.
        // API-only apps must not fail because a strict assets mount cannot be pinned.
    ...
    fn derived_mount_parse_errors_name_their_source_knob() {
        // a serve-rails root whose path contains a comma breaks MountSpec's
