...
/// Optional counting wrapper around the System allocator.
///
/// Counts calls to alloc, alloc_zeroed and realloc, not bytes or deallocations.
/// Relaxed ordering suffices for this statistical counter; it is not used for
/// synchronization. The edge binary registers the allocator, since a library
/// cannot assume ownership of the process-wide allocator choice.
...
/// Optional wall-clock probes for request ingestion and worker-hop stages.
/// These spans include waiting time as well as CPU execution.
///
/// The module, probes and admin endpoint require `hop-timing`. Even with the
/// feature, `OXO_HOP_TIMING=1` is required for Timer::start to record samples.
/// Instrumentation overhead must be considered when interpreting measurements.
...
pub mod hop_timing {
    ...
    // Ingest spans request_filter entry through header collection, validation,
    // admission, body buffering and frame encoding, ending before pool checkout.
    // It is the fifth serialized stage. Response decoding, header construction
    // and downstream writes are outside the worker response-read span.
    ...
    /// Log2 nanosecond buckets. Bucket i covers [2^i, 2^(i+1)); the final
    /// bucket also accumulates larger values. Count and sum retain those values.
    ...
    /// Linear histograms with 64-microsecond bins up to 32.768 ms, alongside
    /// the log2 histograms. The saturated final bin reports overflow count and sum
    /// separately, so consumers can identify unbounded percentile intervals.
    /// Count and sum_nanos include every sample, allowing means to be checked
    /// without estimating values from bin midpoints.
...
/// Hop-timing accounting tests, compiled only with the feature. A single test
/// controls the once-initialized recording gate deterministically.
...
mod hop_timing_tests {
    ...
    fn record_and_report_are_consistent() {
        ...
        // Ingest serializes last, preserving the stage indices and JSON key order.
        assert!(
...
pub struct EdgeCliConfig {
    ...
    /// Downstream HTTP/1.1 connection reuse. The default is one-shot unless the
    /// serve-rails preset enables keepalive; explicit configuration takes precedence.
    ...
    /// Worker-hop wire format: pooled binary `frame` by default, or one-shot
    /// HTTP/1.1 `http`. Validated by the boot resolver.
    pub worker_hop: Option<String>,
    /// Worker dispatch policy, defaulting to least-outstanding. Depth-aware
    /// modes require at most 16 workers; round-robin supports larger fleets.
    pub worker_dispatch: Option<String>,
    /// per-worker admission cap K ("dispatch-on-free"). None = uncapped, the
    /// default; overflow beyond K parks in a shared edge-side FIFO and binds
    ...
    /// Tokio threads for the Pingora proxy service. Defaults to one on hosts
    /// with at most two available processors, otherwise the processor count.
    ...
    /// which would fragment the single-process security counters.
    ...
    /// Production static-routing mode. Disabled by default: GET/HEAD requests
    /// may probe configured mounts, allowing newly deployed files to appear.
    /// With `--prod`, enumerate docroot first path segments at boot and skip
    /// static lookup for unmatched dynamic paths. New top-level static names
    /// require a restart to enter this set. This flag controls static routing;
    /// it does not enable the separate public production exposure mode.
...
impl TrustedProxyPolicy {
    ...
    /// the frame-path variant — the proxy already holds `LoweredHeader`s whose
...
/// Test-only identity resolver re-export for parity with the proxy's direct-public
/// path. Linux gating matches its callers and avoids unused code in stub builds.
...
pub struct LoweredHeader<'a> {
    ...
    /// `None` when `name` is already lowercase (H2 always; most H1 clients) —
...
impl<'a> LoweredHeader<'a> {
    /// arena constructor — name/value/lower live in the request's bump arena
...
pub fn sanitize_lowered_worker_request_headers(
    ...
    // in build_request_frame was ~12 wasted allocations per request.
...
/// the frame path's allocation-free sanitize result — a keep-bitmask over the
...
/// sanitize for the FRAME path — identical validation ladder, rejection set,
...
pub enum EdgeError {
    ...
    NoWorkersConfigured,
    #[error("Pingora edge requires Linux")]
    UnsupportedPlatform,
...
pub enum RequestRejection {
    #[error("WebSocket/Upgrade requires a configured Action Cable route")]
    UpgradeUnsupported,
    #[error("gRPC requires a configured native gRPC sidecar route")]
    GrpcUnsupported,
    #[error("streaming/SSE requires an enabled streaming route")]
...
mod tests {
    ...
    fn edge_cli_rejects_unavailable_production_public_mode() {
        ...
        .expect_err("production public mode must be rejected");
    ...
    fn edge_cli_rejects_public_only_flags_on_loopback_bind() {
        // public-mode / public-identity / trusted-proxy-cidr are only honored on a
