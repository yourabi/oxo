...
/// Worker launch configuration. Classic runs the sibling `oxo-worker` binary.
/// Async runs `<bundle> exec <ruby> <script> <socket>` in `app_root`, using absolute,
/// validated paths. Async interpreter selection is separate from the guarded
/// sibling-binary override setting and does not resolve Ruby through PATH.
#[derive(Debug, Clone)]
...
pub struct ServiceConfig {
    ...
    /// the worker launch surface (see `WorkerLaunch`).
    pub worker_launch: WorkerLaunch,
    /// First-boot readiness budget: 15 seconds for classic, 120 seconds for async.
    /// `OXO_WORKER_READY_TIMEOUT_MS` overrides it; zero is rejected.
    pub worker_ready_timeout: Duration,
    /// Respawn readiness budget, with a 30-second async default to bound the time
    /// the monitor can spend waiting for a restarting worker.
    pub worker_respawn_ready_timeout: Duration,
    /// Worker TERM drain stage: one second for classic; for async, the worker's
    /// drain deadline plus 150 ms so the supervisor allows that drain to complete.
...
impl ServiceConfig {
    ...
    fn from_cli(edge_cli: EdgeCliConfig) -> Result<Self, ServiceError> {
        ...
        // the worker kind decides the whole launch surface. Under async the
        ...
        // the SERVICE owns the runtime dir. Created (or verified
        ...
        // a set-but-garbage frame-pool idle ceiling must
...
/// `OXO_WORKER_KIND` (classic|async; default classic; typo fails closed)
...
/// Read per-kind first-boot readiness budgets and reject zero overrides.
...
/// the respawn budget covers a WARM app boot, not a cold one — a
...
/// async DERIVES the supervisor's worker drain stage from the
...
/// the frame-pool idle ceiling is forwarded to the edge
...
/// Validate the idle-pool selector. Only `owned` or unset is supported;
/// other values fail explicitly rather than silently selecting an implementation.
...
/// the service owns the socket runtime dir. Missing dir: created
...
impl ServiceRunner {
    ...
    fn run(self) -> Result<(), ServiceError> {
        ...
        let Some(mut workers) = spawn_worker_pool(&self.config)? else {
            ...
            // pool is already torn down, so exit gracefully.
...
// instead of reporting a startup error.
fn spawn_worker_pool(config: &ServiceConfig) -> Result<Option<Vec<ManagedWorker>>, ServiceError> {
    // Async workers boot concurrently; classic workers boot sequentially.
    // Both paths share readiness validation and child ownership.
...
/// Boot async workers concurrently under a rolling pool deadline. Attach each
/// child's readiness reader immediately so startup output cannot block a sibling.
/// Each READY resets the straggler deadline; a child exit is reported promptly.
/// A drain signal tears down the partially booted pool.
fn spawn_worker_pool_batched(
    ...
    while !booting.is_empty() {
        if SHUTDOWN_REQUESTED.load(Ordering::SeqCst) {
            fail(&mut booting, &mut ready);
            return Ok(None); // Let the monitor run the graceful drain.
        }
...
fn spawn_worker_slot(
    ...
    // the launch surface decides the command; EVERYTHING else — env_clear, the
    ...
    let (mut command, spawn_path) = match &config.worker_launch {
        ...
        WorkerLaunch::Async {
            ...
            // absolutely ( — `bundle exec ruby` would PATH-resolve it);
...
/// the readiness reader, attachable AT SPAWN TIME. The batched
...
// monitor loop run the graceful shutdown path instead of blocking here.
fn wait_for_worker_ready(
    ...
    loop {
        // honor a drain signal promptly instead of blocking up to the full readiness
...
fn monitor_children(
    ...
    loop {
        ...
        // a worker restart that fails (spawn error, or the respawned worker never
...
fn restart_worker(
    ...
    // a drain requested while a worker is crash-looping must be honored within roughly
    ...
    // respawns run on the SHORTER warm-boot budget — a
...
/// perform the bounded edge restart. No marker → nothing to do.
#[cfg(feature = "acme")]
...
fn drain_service(
    ...
    // async derives this stage from the worker's own drain
...
/// Build the async worker's allowlisted environment. Inject `OXO_ASYNC=1`,
/// resolve an existing Gemfile, and forward body limits, Rack lint, database
/// settings and the worker drain deadline. Sensitive values are redacted in logs.
/// Classic-only thread and streaming controls are not forwarded. A vendored app
/// bundle needs no HOME; user-directory gem installations require explicit
/// HOME/GEM_HOME/GEM_PATH entries in `OXO_APP_ENV_ALLOW`.
fn collect_async_worker_env(
    ...
    // Forward the optional worker timing output directory through the cleared
    // child environment. Timing collection is disabled when the setting is absent.
    if let Some(dir) = optional_string_env("OXO_WORKER_TIMING_DIR")? {
        push_env_value(&mut envs, "OXO_WORKER_TIMING_DIR", OsStr::new(&dir));
    }
    // Forward the optional tail threshold with the timing output directory.
    // The worker validates nonnegative nanoseconds; unset keeps its default.
    ...
    // Forward the optional request-order output directory and fair-read mode.
    // Unset or empty selects `yield2`; `off` disables the scheduling step.
    ...
    // Forward the optional worker scheduling policy; the worker logs the achieved value.
...
fn collect_edge_env(worker_launch: &WorkerLaunch) -> Vec<(OsString, OsString)> {
    // Forward optional native-route and CPU-injection controls only in builds
    // with `edge-bench`. The child reads these once at startup.
    #[allow(unused_mut)]
    ...
    // Forward the timing gate explicitly because the edge child's environment
    // is cleared. This setting is available only with `hop-timing`.
    #[cfg(feature = "hop-timing")]
    if let Ok(value) = env::var("OXO_HOP_TIMING") {
        vars.push((OsString::from("OXO_HOP_TIMING"), OsString::from(value)));
    }
    // Forward the optional scheduler-delay stamp through the cleared child environment.
    #[cfg(feature = "hop-timing")]
    if let Ok(value) = env::var("OXO_HOP_STAMP") {
        vars.push((OsString::from("OXO_HOP_STAMP"), OsString::from(value)));
    }
    // Forward the dispatch setting explicitly through the cleared child environment.
    if let Ok(value) = env::var("OXO_WORKER_DISPATCH") {
        vars.push((OsString::from("OXO_WORKER_DISPATCH"), OsString::from(value)));
    }
    // Forward the per-worker admission cap through the cleared child environment.
    if let Ok(value) = env::var("OXO_EDGE_WORKER_CAP") {
        vars.push((OsString::from("OXO_EDGE_WORKER_CAP"), OsString::from(value)));
    }
    // Forward the optional reaper toggle through the cleared child environment.
    if let Ok(value) = env::var("OXO_HOP_REAPER") {
        vars.push((OsString::from("OXO_HOP_REAPER"), OsString::from(value)));
    }
    // Forward the frame-pool idle ceiling. Configuration validation also runs
    // under `--check-config`, so malformed values cannot silently use a default.
    ...
    // Forward the resolved worker kind so the edge's retry policy agrees with
    // the supervisor's selected worker implementation.
...
fn collect_edge_args(
    ...
    // Translate edge-only identity, fairness and long-lived limits into child
    // arguments. The cleared child environment cannot carry these implicitly.
    ...
    // fail-closed cross-check: trusted-proxy CIDRs are only consumed when the
...
fn resolve_edge_bind(cli: &EdgeCliConfig) -> Result<SocketAddr, ServiceError> {
    match (cli.http_bind, cli.https_bind) {
        (Some(_), Some(_)) => Err(ServiceError::InvalidEnv {
            name: "argv",
            message:
                "--http-bind and --https-bind are mutually exclusive for the application listener"
                    .to_string(),
        }),
        (Some(bind), None) | (None, Some(bind)) => Ok(bind),
        (None, None) => {
            required_string("OXO_EDGE_BIND")?
                .parse()
                .map_err(|err| ServiceError::InvalidEnv {
                    name: "OXO_EDGE_BIND",
...
mod tests {
    ...
    // every worker-kind/async-launch env scenario in ONE test (the vars are
    ...
    fn worker_launch_resolver_fails_closed_and_validates_the_async_surface() {
        ...
        // The edge env carries the kind only under async (the zero-replay gate).
    ...
    // Reject group/other-writable async scripts because the service executes them.
    // Use /tmp so Linux permission bits are meaningful: WSL-mounted Windows paths
    // can bypass the permission check. This test targets Linux, matching the
    // implementation; other platforms use the non-Linux validation stub.
    #[cfg(target_os = "linux")]
    #[test]
    ...
    // timeout/drain knobs — defaults per kind, overrides, zero rejected — in
    ...
    fn worker_timeout_and_drain_knobs_default_per_kind_and_reject_zero() {
        ...
        assert!(
            ...
            "invalid pool-idle must fail boot instead of selecting a default"
        );
    ...
    // enumerate the async worker script's actual ENV reads
    ...
    fn service_drain_grace_must_fit_edge_sidecar_and_worker_windows() {
        ...
        // the async-derived worker stage widens the requirement.
        validate_drain_grace_hierarchy(
            Duration::from_secs(5),
            Duration::from_secs(2),
    ...
    fn service_collects_resolved_edge_args_without_edge_env() {
        ...
        // Feature-enabled tests share process environment. Permit only the explicit
        // diagnostic variables forwarded by collect_edge_env, including a timing gate
        // set by another test; arbitrary environment values must remain excluded.
    ...
    fn collect_edge_env_forwards_the_reaper_toggle() {
        // The reaper setting must survive the cleared child environment. Process
        // environment is shared across tests: set, check, then restore the value.
    ...
    fn service_forwards_trusted_proxy_and_long_lived_edge_args() {
        // Assert that identity, fairness and long-lived settings reach the child
        // with their supplied values, not merely that each flag is present.
        let cli = EdgeCliConfig::parse_from_args([
            "oxo-pingora-service",
            "--https-bind",
            "0.0.0.0:8443",
    ...
    fn service_rejects_trusted_proxy_cidrs_without_trusted_proxy_identity() {
        // fail-closed cross-check: CIDRs set under a public mode while the identity
