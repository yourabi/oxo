//! # oxo-edge — the legacy hyper HTTP edge
//!
//! This crate is shipped history from the -line. The current public-edge
//! runtime is `oxo-pingora-edge`; keep comments here scoped to the legacy
//! hyper path.
//!
//! This edge accepts client
//! connections (hyper, HTTP/1.1), turns each request into a normalized
//! [`oxo_core::RackRequest`], hands it to a [`oxo_core::RackHandler`], and
//! writes the [`oxo_core::RackResponse`] back. It performs the normalization
//! the panel review asked for:
//!
//! * a hard request-body cap (413 beyond [`Config::max_body_bytes`]),
//! * dropping underscore-bearing header names (anti-`HTTP_X_FORWARDED_*` forgery),
//! * re-emitting **canonical framing** to any upstream (the supervised worker) rather
//!   than forwarding client framing — closing the front/back desync gap.
//!
//! adds the edge-hardening suite: a connection-count cap (a `Semaphore` with
//! acquire-before-accept backpressure), a head-read/idle-keepalive timeout, a
//! per-request (handler + body-read) timeout → 504, header count/buffer caps, an
//! optional absolute connection-lifetime deadline (the only bound covering the
//! response-write phase), transient-`accept()` resilience, and an observability
//! floor. TLS and HTTP/2 are deferred in this legacy hyper path. The current
//! Pingora path has its own gated TLS/H1+H2 support and public smoke-beta rules.
...
/// for an ephemeral port in the legacy hyper path.
