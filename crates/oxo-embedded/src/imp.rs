//! The real in-process handler. Compiled only with `--features ruby` (which links
//! libruby via magnus/rb-sys). See the crate docs for the init ceremony and the
//! shared-fate blast-radius caveat.
//!
//! NOTE: rejects this path as native Windows runtime support. The observed
//! Windows setup is MSVC Rust plus MinGW/UCRT Ruby headers, and forcing the Ruby
//! feature fails in rb-sys/bindgen. The default Windows build still links no
//! Ruby. This path is runtime-smoked on Linux CI as legacy coverage and is not
//! the standalone S1 worker contract.
