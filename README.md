...
Oxo is **early / alpha**. Public bind requires explicit `alpha` or `smoke-beta`
configuration; public `production` mode is unavailable. Read
...
| `oxo-edge` | Hyper HTTP/1.1 edge with supervised TCP worker handlers. |
...
For test prerequisites and suite commands, see [Correctness tests](test/README.md).

The Pingora edge and native worker runtimes target Linux. The embedded-Ruby crate needs a working
